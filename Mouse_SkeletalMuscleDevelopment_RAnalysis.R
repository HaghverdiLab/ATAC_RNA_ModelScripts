...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber/Myofiber_RNA_UMAP_RNA.svg", width = 7, height = 7)
DimPlot(so, reduction = "umap.RNA", group.by = "seurat_clusters", label = TRUE, label.size = 5, repel = TRUE) + 
  NoLegend() + ggtitle("RNA UMAP, colored by RNA clusters") + xlim(-15, 15) + ylim(-14, 12) +
  scale_colour_discrete(drop=TRUE, limits = levels(so$seurat_clusters))
dev.off()

svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber/Myofiber_ATAC_UMAP_RNA.svg", width = 7, height = 7)
DimPlot(so, reduction = "umap.atac", group.by = "seurat_clusters", label = TRUE, label.size = 5, repel = TRUE) + 
  NoLegend() + ggtitle("ATAC UMAP, colored by RNA clusters") + xlim(-13, 13) + ylim(-13, 13) +
  scale_colour_discrete(drop=TRUE, limits = levels(so$seurat_clusters))
dev.off()

##### E14
Convert("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_e14/muscle_e14_balanced200_integrated_celltypes/atac_rna_test_preds.h5ad", dest = "h5seurat", overwrite = TRUE)
musclee14_test <- LoadH5Seurat("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_e14/muscle_e14_balanced200_integrated_celltypes/atac_rna_test_preds.h5seurat", assays = "RNA")

musclee14_test_cells <- read.table(
  file = "/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_e14/muscle_e14_balanced200_integrated_celltypes/atac_rna_test_cells.txt",
  col.names = c("cells")
)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_E14_Original_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_E14_Test_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_E14_Inferred_Transferred.svg", width = 7, height = 7)
...
e14_ref_celltypes_counts <- data.frame(name = names(table(so_e14$celltype)), count = as.numeric(table(so_e14$celltype)))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/E14_Data_Celltypes.svg", width = 7, height = 7)
ggplot(e14_ref_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltype Distribution, E14 Data") +
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_e14_test_cells$celltype))),color = "black") + 
  theme(text = element_text(size=18), axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5), legend.position="none") 
dev.off()

e14_ref_test_celltypes_counts <- e14_ref_celltypes_counts
e14_ref_test_celltypes_counts$count <- sapply(e14_ref_test_celltypes_counts$count, function(x) ifelse(x > 200, 200, x))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/E14_Data_Balanced_Celltypes.svg", width = 7, height = 7)
ggplot(e14_ref_test_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltypes after Balancing, E14 Data") + 
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_e14_test_cells$celltype))),color = "black") +
  theme(text = element_text(size=18),axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="none")
...
#so_e14 <- subset(so, cells = names(which(so$orig.ident == "E14")))
#p4 <- DimPlot(so_e14, reduction = "umap.RNA", group.by = "celltype", label = TRUE, label.size = 3,
#              repel = TRUE) + NoLegend() + ggtitle("Reference annotations")  + xlim(-15, 15) + ylim(-15, 15)

##### E18
Convert("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_e18/muscle_e18_balanced200_integratedcelltypes/atac_rna_test_preds.h5ad", dest = "h5seurat", overwrite = TRUE)
muscleE18_test <- LoadH5Seurat("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_e18/muscle_e18_balanced200_integratedcelltypes/atac_rna_test_preds.h5seurat", assays = "RNA")

muscleE18_test_cells <- read.table(
  file = "/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_e18/muscle_e18_balanced200_integratedcelltypes/atac_rna_test_cells.txt",
  col.names = c("cells")
)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_E18_Original_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_E18_Test_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_E18_Inferred_Transferred.svg", width = 7, height = 7)
...
E18_ref_celltypes_counts <- data.frame(name = names(table(so_E18$celltype)), count = as.numeric(table(so_E18$celltype)))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/E18_Data_Celltypes.svg", width = 7, height = 7)
ggplot(E18_ref_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltype Distribution, E18 Data") +
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_E18_test_cells$celltype))),color = "black") + 
  theme(text = element_text(size=18), axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5), legend.position="none") 
dev.off()

E18_ref_test_celltypes_counts <- E18_ref_celltypes_counts
E18_ref_test_celltypes_counts$count <- sapply(E18_ref_test_celltypes_counts$count, function(x) ifelse(x > 200, 200, x))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/E18_Data_Balanced_Celltypes.svg", width = 7, height = 7)
ggplot(E18_ref_test_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltypes after Balancing, E18 Data") + 
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_E18_test_cells$celltype))),color = "black") +
  theme(text = element_text(size=18),axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="none")
...
E18_euc_dist <- sqrt(rowSums((so_E18_test_cells@reductions$umap.RNA@cell.embeddings - muscleE18_test@reductions$ref.umap@cell.embeddings)^2))
E18_euc_dist_df <- data.frame(eucdist = E18_euc_dist)
sum(sqrt(rowSums((so_E18_test_cells@reductions$umap.RNA@cell.embeddings - muscleE18_test@reductions$ref.umap@cell.embeddings)^2)))
sum(sqrt(rowSums((so_E18_test_cells@reductions$umap.RNA@cell.embeddings - muscleE18_test@reductions$ref.umap@cell.embeddings)^2)))/length(so_E18_test_cells$orig.ident)
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber/Mouse_E18_EucDist.svg", width = 7, height = 7)
ggplot(E18_euc_dist_df, aes(eucdist)) + geom_histogram(binwidth = 0.5,center=0.25,fill = 'sky blue',color = 'black') +
  ggtitle("E18, Ref. UMAP Euclidean Distance between Original and Inferred") +
  xlab("Euclidean Distance") + ylab("Count")
dev.off() 
E18_euc_dist_df$celltype <- so_E18_test_cells$celltype
print(E18_euc_dist_df %>% group_by(celltype) %>% summarise(euc = sum(eucdist)/n(), cells=n())) 

##### P5
Convert("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_p5/muscle_p5_balanced200_integratedcelltypes/atac_rna_test_preds.h5ad", dest = "h5seurat", overwrite = TRUE)
muscleP5_test <- LoadH5Seurat("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_p5/muscle_p5_balanced200_integratedcelltypes/atac_rna_test_preds.h5seurat", assays = "RNA")

muscleP5_test_cells <- read.table(
  file = "/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_p5/muscle_p5_balanced200_integratedcelltypes/atac_rna_test_cells.txt",
  col.names = c("cells")
)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_P5_Original_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_P5_Test_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_P5_Inferred_Transferred.svg", width = 7, height = 7)
...
P5_ref_celltypes_counts <- data.frame(name = names(table(so_P5$celltype)), count = as.numeric(table(so_P5$celltype)))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/P5_Data_Celltypes.svg", width = 7, height = 7)
ggplot(P5_ref_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltype Distribution, P5 Data") +
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_P5_test_cells$celltype))),color = "black") + 
  theme(text = element_text(size=18), axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5), legend.position="none") 
dev.off()

P5_ref_test_celltypes_counts <- P5_ref_celltypes_counts
P5_ref_test_celltypes_counts$count <- sapply(P5_ref_test_celltypes_counts$count, function(x) ifelse(x > 200, 200, x))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/P5_Data_Balanced_Celltypes.svg", width = 7, height = 7)
ggplot(P5_ref_test_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltypes after Balancing, P5 Data") + 
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_P5_test_cells$celltype))),color = "black") +
  theme(text = element_text(size=18),axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="none")
...
P5_euc_dist <- sqrt(rowSums((so_P5_test_cells@reductions$umap.RNA@cell.embeddings - muscleP5_test@reductions$ref.umap@cell.embeddings)^2))
P5_euc_dist_df <- data.frame(eucdist = P5_euc_dist)
sum(sqrt(rowSums((so_P5_test_cells@reductions$umap.RNA@cell.embeddings - muscleP5_test@reductions$ref.umap@cell.embeddings)^2)))
sum(sqrt(rowSums((so_P5_test_cells@reductions$umap.RNA@cell.embeddings - muscleP5_test@reductions$ref.umap@cell.embeddings)^2)))/length(so_P5_test_cells$orig.ident)
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber/Mouse_P5_EucDist.svg", width = 7, height = 7)
ggplot(P5_euc_dist_df, aes(eucdist)) + geom_histogram(binwidth = 0.5,center=0.25,fill = 'sky blue',color = 'black') +
  ggtitle("P5, Ref. UMAP Euclidean Distance between Original and Inferred") +
  xlab("Euclidean Distance") + ylab("Count")
dev.off() 
P5_euc_dist_df$celltype <- so_P5_test_cells$celltype
print(P5_euc_dist_df %>% group_by(celltype) %>% summarise(euc = sum(eucdist)/n(), cells=n())) 

##### Adult
Convert("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_adult/muscle_adult_balanced200_integratedcelltypes/atac_rna_test_preds.h5ad", dest = "h5seurat", overwrite = TRUE)
muscleAdult_test <- LoadH5Seurat("/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_adult/muscle_adult_balanced200_integratedcelltypes/atac_rna_test_preds.h5seurat", assays = "RNA")

muscleAdult_test_cells <- read.table(
  file = "/fast/AG_Haghverdi/Siddharth_Annaldasula/projects/atac_rna_enc_mouse/muscle_adult/muscle_adult_balanced200_integratedcelltypes/atac_rna_test_cells.txt",
  col.names = c("cells")
)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_Adult_Original_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_Adult_Test_Truth.svg", width = 7, height = 7)
...
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Mouse_Adult_Inferred_Transferred.svg", width = 7, height = 7)
...
Adult_ref_celltypes_counts <- data.frame(name = names(table(so_Adult$celltype)), count = as.numeric(table(so_Adult$celltype)))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Adult_Data_Celltypes.svg", width = 7, height = 7)
ggplot(Adult_ref_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltype Distribution, Adult Data") +
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_Adult_test_cells$celltype))),color = "black") + 
  theme(text = element_text(size=18), axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5), legend.position="none") 
dev.off()

Adult_ref_test_celltypes_counts <- Adult_ref_celltypes_counts
Adult_ref_test_celltypes_counts$count <- sapply(Adult_ref_test_celltypes_counts$count, function(x) ifelse(x > 200, 200, x))
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber_Balanced/Adult_Data_Balanced_Celltypes.svg", width = 7, height = 7)
ggplot(Adult_ref_test_celltypes_counts, aes(x = name, y = count)) + 
  ggtitle("Celltypes after Balancing, Adult Data") + 
  xlab(NULL) +
  ylab("Cell Count") + 
  geom_col(aes(reorder(name, -count), fill = levels(as.factor(so_Adult_test_cells$celltype))),color = "black") +
  theme(text = element_text(size=18),axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), legend.position="none")
...
Adult_euc_dist <- sqrt(rowSums((so_Adult_test_cells@reductions$umap.RNA@cell.embeddings - muscleAdult_test@reductions$ref.umap@cell.embeddings)^2))
Adult_euc_dist_df <- data.frame(eucdist = Adult_euc_dist)
sum(sqrt(rowSums((so_Adult_test_cells@reductions$umap.RNA@cell.embeddings - muscleAdult_test@reductions$ref.umap@cell.embeddings)^2)))
sum(sqrt(rowSums((so_Adult_test_cells@reductions$umap.RNA@cell.embeddings - muscleAdult_test@reductions$ref.umap@cell.embeddings)^2)))/length(so_Adult_test_cells$orig.ident)
svg("/fast/AG_Haghverdi/Siddharth_Annaldasula/Section2_Myofiber/Mouse_Adult_EucDist.svg", width = 7, height = 7)
ggplot(Adult_euc_dist_df, aes(eucdist)) + geom_histogram(binwidth = 0.5,center=0.25,fill = 'sky blue',color = 'black') +
  ggtitle("Adult, Ref. UMAP Euclidean Distance between Original and Inferred") +
  xlab("Euclidean Distance") + ylab("Count")
dev.off() 
Adult_euc_dist_df$celltype <- so_Adult_test_cells$celltype
print(Adult_euc_dist_df %>% group_by(celltype) %>% summarise(euc = sum(eucdist)/n(), cells=n())) 
